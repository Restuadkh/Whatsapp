...
app.post('/login', (req, res) => {
    ...
    db.query(sql, [username, password], (err, results) => {
        ...
        if (results.length === 1) {
            ...
            res.status(200).json({status:'User Ditemukan',username: username,password: password}); 
        } else {
            res.status(404).json({status:'User Tidak Ditemukan',username: username,password: password});
        }
    });
});
